...
const ContactBtn = ({ theme }) => {
  ...
  return (
    <button
      className={`${
        isDark ? "bg-white text-cDark" : "bg-cDark text-white"
      } h-[70px] w-[70px] text-4xl rounded-full border-none fixed bottom-8 right-10 z-50 mail__btn flex justify-center items-center hover:scale-110 active:scale-90 transition-all`}
    >
      <MdMail />
    </button>
  );
  ...
  return (
    <>
      <section
        id='hero'
        className={`${
          theme === "dark" ? "text-white" : "text-cDark"
        } transition-all header h-[90vh]`}
      >
        <div className={`${modal ? "hidden" : ""} -mt-28`}>
          ...
          <a
            href='#projects'
            className={`${modal ? "opacity-0 hidden" : ""} scroll`}
          >
            <div
              className={`${
                theme === "dark" ? "scroll__icon--white" : ""
